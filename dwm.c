...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, iscentered, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isfakefullscreen, isterminal, noswallow, nopreserve;
...
static int resizebarwin(Monitor *m);
...
static int stextw = 0;       /* width of stext */
static int screen;
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		else if (ev->x > selmon->ww - stextw - getsystraywidth())
			click = ClkStatusText;
		else
			click = ClkWinTitle;
...
deck(Monitor *m) {
	...
	for(i = my = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if(i < m->nmaster) {
			...
			resize(c, m->wx + mw - c->bw, m->wy, m->ww - mw - c->bw, m->wh - (2*c->bw), False);
}
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	w += TEXTW(text); /* finalize including padding */
	text = p;
		...
		if (text[i] == '^' && !isCode) {
			...
			while (text[++i] != '^') {
				if (text[i] == 'c' || text[i] == 'b') {
					int ClrIndex = (text[i] == 'c')? ColFg:ColBg;
					char buf[8];
					memcpy(buf, (char*)text+i+1, 7);
					buf[7] = '\0';
					drw_clr_create(drw, &drw->scheme[ClrIndex], buf);
					i += 7;
				} else if (text[i] == 'd') {
					drw->scheme[ColFg] = scheme[SchemeNorm][ColFg];
					drw->scheme[ColBg] = scheme[SchemeNorm][ColBg];
	...
	drw_text(drw, x, 0, TEXTW(text) - lrpad, bh, 0, text, 0);
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		for (c = m->clients; c; c = c->next) {
			if (c->tags & 1 << i) {
				drw_rect(drw, x + ((indc<3)? boxs+1:w-boxs-boxw-1),
						1 + boxs + (indc%3)*(boxw+boxs), boxw, boxw,
						selmon->sel == c, urg & 1 << i);
				indc++;
			}
		}
		// if (occ & 1 << i)
...
manage(Window w, XWindowAttributes *wa)
{
	...
	updatesizehints(c);
	updatewmhints(c);
	if(!c->nopreserve){ // restore previous assignment of the client
		Monitor *m;
		int di;
		unsigned long dl;
...
tile(Monitor *m)
{
	...
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->wx + mw - c->bw, m->wy + ty, m->ww - mw - c->bw, h - (2*c->bw), 0);
			if (ty + HEIGHT(c) < m->wh){
				ty += HEIGHT(c) - c->bw;
				sfacts -= c->cfact;
			}
